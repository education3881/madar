...
> **Lessons carried forward — 2026-08-05 (Manager, monthly review of the last ~30 days).** The Verifier's eight verdicts and the guidebook's rulings #19–#28 changed what "sourcing reality" means at your gate. Three additions to your practice, all in service of content quality:
>
> 1. **A figure now travels with seven named axes — and your Test 1 checks them.** The month's failures (#21–#25) were not fabricated sources; they were *real* figures carried with the wrong companion fact. Every load-bearing number arrives with: administration/coverage (#12/#12b), funnel stage (#14), **vintage** (#21 — is it the system-of-record's standing figure, or a later recount?), **scope** (#22 — does a shared qualifier govern both falls, or one?), **direction** (#23 — "almost" vs "over" belong to specific vintages), **position** (#24 — a series point carries its column and base note), and **condition** (#25 — an eligibility rate is not an event-given-eligibility rate). When you read a draft, read *at the axis, not just at the number*: does the sentence carry the qualifier the source attached, to the same list the source attached it to? This is now part of Test 1, not a separate pass.
> 2. **The guardrail must be in the sentence (#19).** For every risk the recon's §6 flags, the draft must *carry the fence in the prose* — comparability limits stated where the comparison is made, "announced ≠ measured" labelled at the figure. Silent avoidance is a fail: an approve-for-the-wave requires the fence be visible to the reader, not merely known to us.
> 3. **Your approval is the editorial gate, not the ship gate.** Since 2026-07-12 the Verifier (persona #11) independently re-traces every load-bearing figure through a *second, differently-routed channel* before the `approved: true` flip (#27). Your five-test verdict is "APPROVE for the wave"; the flip is earned by the Verifier's verdict on file (#18 — the flag is the only hold that survives `git add -A`). Do not treat your approval as clearance to ship; treat it as clearance to *enter verification*. Check dek/schema caps at verdict time (dek ≤ 200 code points including Arabic tashkeel) — the month showed these are cheapest caught at your desk, not at build.
