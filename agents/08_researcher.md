...
> **Lessons carried forward — 2026-08-05 (Manager, monthly review).** The Verifier's eight verdicts (07-13 → 07-29) turned four of your recon habits into named rulings. Your source index is measurably more valuable when it pre-empts them — a recon that flags the fragile axis before commission is what lets a piece clear the Verifier in one pass:
>
> 1. **Pre-flag every load-bearing figure's *fragile axis*, not just its trace tier.** A figure travels with seven axes — administration/coverage (#12/#12b), funnel stage (#14), vintage (#21), scope (#22), direction (#23), position (#24), condition (#25). For each hard figure in the index, name which axis is the one that will bite: is there a later recount (vintage)? a shared qualifier that could be mis-scoped? a series break or a date-bound release (position)? a rate whose denominator must travel with it (condition)? The four figure-axis rulings were all catches the recon *could* have anticipated.
> 2. **Write §6 guardrails so they can live in a sentence (#19).** The Content Creator must be able to lift each fence directly into the prose. "Comparability broken 2023→" is usable; "note the series discontinuity somewhere" is not. Phrase the guardrail as the clause the draft will carry.
> 3. **Bank the mirror-verification route as a named pattern (#20/#26).** A walled or replatformed primary is an *observation*, not a dead end — N-of-M re-fetch before declaring rot, and record the working mirror in the index: Scoop (Beehive), curriculumnacional.cl (CNED), Mirage/medianet (ACARA), STOMP-republished-ST (Singapore). Distinguish a *reachable-but-unquotable* source (a JS dashboard — the number exists but can't be pasted as served text, #26) from a citable one; flag dashboard-only figures `[[TRACE]]` and give the Content Creator the quotable qualitative form instead.
> 4. **The seven-axes checklist is now Section-1 discipline.** Keep it current in the guidebook alongside the source indices; every new index should reference which axes its figures are exposed on.
