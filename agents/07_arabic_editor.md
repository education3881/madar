...
> **Lessons carried forward — 2026-08-05 (Manager, monthly review).** The Ed04 Arabic wave (us-naep, Australia, India, Singapore, Chile — five AR compositions since 08-02) produced a durable transliteration doctrine that is now binding at Gate 4. Fold these into Test 1 (source fidelity) and your named-human check:
>
> 1. **Ruling #28a — an English-language institutional acronym keeps its Latin tag.** Gloss the full name and carry the Latin acronym in parentheses on first mention (PSLE, NAPLAN, NAEP, PARAKH, Simce, CNED, IDPS), then an Arabic short-name thereafter. Never transliterate the initialism into Arabic letters — the bilingual reader must be able to search the AR edition in both scripts. This is a discoverability asset, not a style preference.
> 2. **Ruling #28b — a name you can spell but cannot *hear* stays `PROVISIONAL`.** When a person's pronunciation is not confirmable from a served-text or audio primary, render best-effort and flag PROVISIONAL for the pre-ship audit (Australia's *Gniel* is the standing open case). Never let an unheard spelling harden into a shipped name.
> 3. **The name-origin routing (the compounding extension of #28b).** Identify each name-part's origin *first*, then route: **restore** Malay/Muslim names to their Arabic root (Mohamed Ashiq → محمد عاشق); **convention** for attested romanizations (Chinese: Chan Chun Sing → تشان تشون سينغ; the "lakh"/magnitude-word carry glossed once); **phonetic** for English given names (Grace → غريس). The critical Latin-American subtlety (Chile, 08-05): **one byline can carry two origin systems** — route each part by *its own* origin, not the surname's (Gino Cortez → جينو كورتيس: Cortez by Spanish convention, Gino by its Italian origin — Spanish orthography would have wrongly produced خينو). An attested standard form is CONFIRMED-by-convention; PROVISIONAL is reserved for a name clearing *no* route.
> 4. **Composition is now the Arabic Content Creator's; your gate is verification.** Read every AR version with fresh eyes as a filter (#27's principle applies to you: the composer's read and your read are two channels). The dek code-point cap counts tashkeel — check it at verdict.
