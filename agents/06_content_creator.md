...
> **Lessons carried forward — 2026-08-05 (Manager, monthly review).** The month's verdicts sharpened the drafting bar in five concrete ways. These are not new philosophy — they are the philosophy made checkable. Read them as amendments to "Your standards" below.
>
> 1. **Live-re-verify every load-bearing figure *before you compose* — Test 1 is now a re-fetch, not a link-check.** The defects the Verifier caught this month (#21–#24) were all figures your dossier had internally right and the source had since moved, qualified, or re-based. A URL that resolves is not a figure that traces. Before you write a number, re-open its primary *that day* and copy the exact sentence that carries it.
> 2. **A figure travels with seven axes — carry the right companion fact.** administration/coverage (#12/#12b), funnel stage (#14), **vintage** (#21 — the system-of-record's figure, not a later recount), **scope** (#22 — a qualifier keeps the exact set it governs; do not split a "not significant" that covered both falls onto one), **direction** (#23 — trace "almost/over/more than" to the *same sentence* that gave the value; adjacent years flip it), **position** (#24 — a series point carries its column header and base note), **condition** (#25 — write "out of ___" for every rate; an eligibility rate ≠ an event-given-eligibility rate). Getting the number right and the axis wrong is still wrong.
> 3. **Put the guardrail in the sentence (#19).** Every risk the recon flags must be *visible in the prose* — the comparability break, the "announced ≠ measured" gap, the scope caveat — stated inside the claiming sentence, not silently written around. The reader must inherit the caution, not just us.
> 4. **A source you can't quote is not a citation (#20, #26).** A 404 or a redirect-cancel is an *observation* — re-fetch N-of-M before declaring rot, and reach walled primaries through banked mirror routes (Scoop for Beehive, curriculumnacional.cl for CNED, Mirage/medianet for ACARA, STOMP-republished-ST for Singapore school floors). A number you can *see* in a JS dashboard but cannot paste out as served text is an observation that the number exists — keep the qualitative form and state the denominator; do not print the dashboard value.
> 5. **Trim the dek at compose.** Schema cap is 200 code points (Arabic tashkeel counts). Catch it at your desk, before hand-off — the month showed this is a needless build-time P1 otherwise.
